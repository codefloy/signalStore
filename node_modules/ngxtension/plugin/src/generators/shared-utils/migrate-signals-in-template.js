...
exports.migrateTemplateVariablesToSignals = void 0;
const bundled_angular_compiler_1 = require("@angular-eslint/bundled-angular-compiler");
function migrateTemplateVariablesToSignals(template, convertedSignals) {
    const parsedTemplate = new bundled_angular_compiler_1.HtmlParser().parse(template, 'template.html', {
        preserveLineEndings: true,
        tokenizeBlocks: true,
    });
    const sortedSignalsFromLongest = Array.from(convertedSignals).sort((a, b) => b.length - a.length);
    ...
    visitor.elements.forEach((element) => {
        ...
        if (type === 'interpolation') {
            const replacedValue = replaceVariablesInsideInterpolation(value, variables);
            template = replaceTemplate(template, replacedValue, start, end, changedOffset);
            ...
            const replacedValue = replaceOnlyIfNotWrappedInQuotesOrPropertyAccess(value, variables);
            template = replaceTemplate(template, replacedValue, start, end, changedOffset);
            changedOffset += replacedValue.length - value.length;
        }
...
exports.migrateTemplateVariablesToSignals = migrateTemplateVariablesToSignals;
        ...
        if (element.attrs.length > 0) {
            for (const attr of element.attrs) {
                if (attr.name.startsWith('[') && attr.name.endsWith(']')) {
                    // two-way binding
                    if (attr.name.startsWith('[(') && attr.name.endsWith(')]')) {
                        const usedVariables = collectUsedVariables(attr.value, this.variables);
                        if (usedVariables.length) {
                            this.elements.push({
    ...
    visitBlock(block, context) {
        if (block.parameters.length > 0) {
            block.parameters.forEach((param) => {
                const usedVariables = collectUsedVariables(param.expression, this.variables);
                if (usedVariables.length) {
                    this.elements.push({
...
function collectUsedVariables(value, variables) {
    const usedVariables = [];
...
function replaceVariablesInsideInterpolation(value, variables) {
 ...
 */
function replaceOnlyIfNotWrappedInQuotesOrPropertyAccess(value, variables) {
...
//# sourceMappingURL=migrate-signals-in-template.js.map